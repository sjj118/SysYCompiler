...
public:
    ...
    void dumpRISC(std::ostream &os) const;
};
...
public:
    ...
    virtual void dumpRISC(std::ostream &os, const TiggerFunc *func) const = 0;
};
...
public:
    ...
    void dump(std::ostream &os) const {
        os << "f_" + ident_ << " [" << arg_num_ << "] [" << slot_num << "]" << std::endl;
        for (const auto &stmt:stmts_)stmt->dump(os);
        os << "end " << "f_" + ident_ << std::endl << std::endl;
    }
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  " << dst_->name() << " = " << lhs_->name() << " " << op2str(op_) << " " << rhs_->name() << std::endl;
    }

    void dumpRISC(std::ostream &os, const TiggerFunc *func) const override;
};
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  " << dst_->name() << " = " << op2str(op_) << rhs_->name() << std::endl;
    }
...
private:
    ...
    std::shared_ptr<TiggerNum> dst_offset_, val_offset_;
public:
    TiggerAssignStmt(std::shared_ptr<TiggerReg> dst, std::shared_ptr<TiggerValue> val,
                     std::shared_ptr<TiggerNum> dst_offset = nullptr,
                     std::shared_ptr<TiggerNum> val_offset = nullptr) :
    ...
    [[nodiscard]] const std::shared_ptr<TiggerNum> &dst_offset() const { return dst_offset_; }

    [[nodiscard]] const std::shared_ptr<TiggerNum> &val_offset() const { return val_offset_; }
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  " << "if " << lhs_->name() << " " << op2str(op_) << " " << rhs_->name()
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  store " << reg_->name() << " " << offset_->name() << std::endl;
    }
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  load " << from_->name() << " " << reg_->name() << std::endl;
    }
...
public:
    ...
    void dump(std::ostream &os) const override {
        os << "  loadaddr " << from_->name() << " " << reg_->name() << std::endl;
    }
...
public:
    ...
    void dump(std::ostream &os) const {
        ...
        for (const auto &func:funcs_)func->dump(os);
    }
